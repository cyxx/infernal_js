...
function clear_screen(){screen8.fill(0)}function draw_object(a,b,c){var d=get_object_width(a),e=get_object_height(a);a=get_object_addr(a);for(var f=0;f<e;++f)for(var h=(c+f)*SCREEN_W+4*b,k=0;k<d;++k){var g=snapshot[a++];screen8[h++]^=(g>>3&1)<<1|g>>7&1;screen8[h++]^=(g>>2&1)<<1|g>>6&1;screen8[h++]^=(g>>1&1)<<1|g>>5&1;screen8[h++]^=(g&1)<<1|g>>4&1}}function draw_object2(a,b,c){console.log("draw_object2 not implemented");draw_object(a,b,c)}var snapshot=Array(49152);
...
function run_threads(){for(thread_num=0;256>thread_num;++thread_num)for(bytecode_offset=threads_bytecode[thread_num];0!=bytecode_offset;){opcode=snapshot[bytecode_offset];if(78==opcode){break_flag=1;return}if(90==opcode)break;bytecode_offset+=1;opcodes[opcode]();if(77==opcode||79==opcode)break}}var keys,score,highscore=0,boxes,hunger,lifes,player_disabled,var_num,keys_flag,player_jumping,ticks,screen_num,break_flag,start_pos,counter1,counter2,counter3,player_direction,player_frame;
...
handle_food();handle_reward();handle_key();handle_box();run_threads();if(10==boxes&&3==screen_num&&2==vars1[0]&&130==vars2[0])state=STATE_COMPLETED;else{if(0!=start_pos){counter2=start_pos;start_pos=0;if(is_key_pressed(KEY_ACTION))return!1;start_pos=counter2;--counter3;if(0==counter3)return!1}0==vars2[0]||1==vars2[0]?(vars2[0]=150,screen_num+=3,change_screen()):151<vars2[0]?(vars2[0]=2,screen_num-=3,change_screen()):0==vars1[0]?(vars1[0]=77,--screen_num,change_screen()):79<vars1[0]&&(vars1[0]=2,screen_num+=
