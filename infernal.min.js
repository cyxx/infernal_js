...
var KEY_UP=1,KEY_RIGHT=2,KEY_DOWN=3,KEY_LEFT=4,KEY_JUMP=5,keyboard=Array(6);function is_key_pressed(a){return keyboard[a]}function release_key(a){keyboard[a]=0}function set_key_pressed(a,b){if(37==a)keyboard[KEY_LEFT]=b;else if(38==a)keyboard[KEY_UP]=b;else if(39==a)keyboard[KEY_RIGHT]=b;else if(40==a)keyboard[KEY_DOWN]=b;else if(32==a||13==a)keyboard[KEY_JUMP]=b}
function set_touch_key(a,b,c){var d=canvas.getBoundingClientRect(),e=d.width/2,f=d.height/2,h=d.left+e/2;d=d.top+f/2;a<h?keyboard[KEY_LEFT]=c:a>h+e?keyboard[KEY_RIGHT]=c:b<d?keyboard[KEY_UP]=c:b>d+f?keyboard[KEY_DOWN]=c:keyboard[KEY_JUMP]=c}
...
function run_threads(){for(thread_num=0;256>thread_num;++thread_num)for(bytecode_offset=threads_bytecode[thread_num];0!=bytecode_offset;){var a=snapshot[bytecode_offset];if(78==a){break_flag=1;return}if(90==a)break;bytecode_offset+=1;opcodes[a]();if(77==a||79==a)break}}var keys,score,highscore=0,boxes,hunger,lifes,infinite_lifes=!1,player_disabled,var_num,keys_flag,player_jumping,ticks,screen_num,break_flag,start_pos,counter3,player_direction,player_frame;
...
function update_game_state(){if(1==player_disabled)return run_threads(),1==break_flag?!1:!0;var a=snapshot[7926+get_next_tile()];0!=(a&240)&&(draw_object(player_frame,vars1[0],vars2[0]),a&16&&(vars1[0]+=1),a&32&&--vars1[0],a&64&&(vars2[0]+=1),a&128&&--vars2[0],draw_object(player_frame,vars1[0],vars2[0]));a=get_next_tile();0!=snapshot[8438+a]&&set_thread_bytecode(snapshot[8438+a],a);1!=keys_flag&&(a=get_next_tile(),0!=snapshot[8182+a]?(set_thread_bytecode(snapshot[8182+a],0),keys_flag=1):0==start_pos&&
(is_key_pressed(KEY_JUMP)?(0!=player_direction?set_thread_bytecode(6,0):set_thread_bytecode(5,0),player_jumping=keys_flag=1):(a=get_next_tile(),0!=(snapshot[7926+a]&12)&&(is_key_pressed(KEY_UP)?(set_thread_bytecode(3,0),keys_flag=1):is_key_pressed(KEY_DOWN)&&(set_thread_bytecode(4,0),keys_flag=1)),is_key_pressed(KEY_RIGHT)?(set_thread_bytecode(1,0),keys_flag=1,player_direction=0):is_key_pressed(KEY_LEFT)&&(set_thread_bytecode(2,0),player_direction=keys_flag=1))));handle_food();handle_reward();handle_key();
handle_box();run_threads();if(10==boxes&&3==screen_num&&2==vars1[0]&&130==vars2[0])state=STATE_COMPLETED;else{if(0!=start_pos){a=start_pos;start_pos=0;if(is_key_pressed(KEY_JUMP))return!1;start_pos=a;--counter3;if(0==counter3)return!1}0==vars2[0]||1==vars2[0]?(vars2[0]=150,screen_num+=3,change_screen()):151<vars2[0]?(vars2[0]=2,screen_num-=3,change_screen()):0==vars1[0]?(vars1[0]=77,--screen_num,change_screen()):79<vars1[0]&&(vars1[0]=2,screen_num+=1,change_screen())}return!0}
...
function draw_panel_hunger(){draw_panel_number(hunger,2,40,184)}function draw_panel_lifes(){draw_panel_number(lifes,2,68,184)}var canvas,timer,audio,INTERVAL=100;
function init(a){canvas=document.getElementById(a);document.onkeydown=function(a){set_key_pressed(a.keyCode,1)};document.onkeyup=function(a){set_key_pressed(a.keyCode,0)};canvas.addEventListener("mousedown",function(a){set_touch_key(a.clientX,a.clientY,1)});canvas.addEventListener("mouseup",function(a){set_touch_key(a.clientX,a.clientY,0)});audio=new Audio("inferrun.ogg");audio.addEventListener("ended",function(){this.currentTime=0;this.play()},!1)}
