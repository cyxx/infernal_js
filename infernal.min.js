...
var PALETTE_CPC=[0,0,0,0,0,128,0,0,255,128,0,0,128,0,128,128,0,255,255,0,0,255,0,128,255,0,255,0,128,0,0,128,128,0,128,255,128,128,0,128,128,128,128,128,255,255,128,0,255,128,128,255,128,255,0,255,0,0,255,128,0,255,255,128,255,0,128,255,128,128,255,255,255,255,0,255,255,128,255,255,255],palette32=Array(4);function set_palette_color(a,b){b*=3;palette32[a]=-16777216|PALETTE_CPC[b+2]<<16|PALETTE_CPC[b+1]<<8|PALETTE_CPC[b]}var SCREEN_W=320,SCREEN_H=200,screen8=Array(SCREEN_W*SCREEN_H);
function clear_screen(){screen8.fill(0)}function draw_object(a,b,c){var d=get_object_width(a),e=get_object_height(a);a=get_object_addr(a);for(var f=0;f<e;++f)for(var h=(c+f)*SCREEN_W+4*b,k=0;k<d;++k){var g=snapshot[a++];screen8[h++]^=(g>>3&1)<<1|g>>7&1;screen8[h++]^=(g>>2&1)<<1|g>>6&1;screen8[h++]^=(g>>1&1)<<1|g>>5&1;screen8[h++]^=(g&1)<<1|g>>4&1}}function draw_object2(a,b,c){draw_object(a,b,c)}var snapshot=Array(49152);function get_object_width(a){return snapshot[6902+a]}
function get_object_height(a){return snapshot[7158+a]}function get_object_addr(a){return snapshot[6390+a]<<8|snapshot[6646+a]}var thread_num,threads_bytecode=Array(256),bytecode_offset,vars1=Array(60),vars2=Array(60);function clear_vars(){vars1.fill(0);vars2.fill(0)}function get_bytecode_addr(a){return snapshot[7414+a]<<8|snapshot[7670+a]}function clear_threads(){threads_bytecode.fill(0)}function set_thread_bytecode(a,b){threads_bytecode[b]=get_bytecode_addr(a)}
...
function run_threads(){for(thread_num=0;256>thread_num;++thread_num)for(bytecode_offset=threads_bytecode[thread_num];0!=bytecode_offset;){opcode=snapshot[bytecode_offset];if(78==opcode){break_flag=1;return}if(90==opcode)break;bytecode_offset+=1;opcodes[opcode]();if(77==opcode||79==opcode)break}}var keys,score,highscore=0,boxes,hunger,lifes,player_disabled,var_num,keys_flag,player_jumping,ticks,screen_num,break_flag,start_pos,counter3,player_direction,player_frame;
...
function pause(){if(timer)return audio.pause(),clearInterval(timer),timer=null,!0;audio.play();timer=setInterval(tick,INTERVAL);return!1}function reset(){clear_vars();set_palette_color(0,0);set_palette_color(1,6);state=STATE_TITLE;timer&&clearInterval(timer);timer=setInterval(tick,INTERVAL)}function mute(){if(!audio.paused)return audio.pause(),!0;audio.play();return!1}
function update_screen(){for(var a=canvas.getContext("2d"),b=a.getImageData(0,0,SCREEN_W,SCREEN_H),c=new Uint32Array(b.data.buffer),d=0;d<SCREEN_W*SCREEN_H;++d)c[d]=palette32[screen8[d]];a.putImageData(b,0,0)}function load_snapshot(a){for(var b=0;49152>b;++b)snapshot[b]=a.charCodeAt(b)&255;reset()}var STATE_TITLE=1,STATE_GAME=2,STATE_COMPLETED=3,state,prev_state;
function tick(){state!=prev_state&&(state==STATE_TITLE?(clear_screen(),set_palette_color(2,2),set_palette_color(3,26),clear_threads(),set_thread_bytecode(130,10)):state==STATE_GAME?(initialize(),restart()):state==STATE_COMPLETED&&(clear_screen(),clear_tilemap(),clear_threads(),set_thread_bytecode(132,10),player_direction=200),prev_state=state);state==STATE_TITLE?(run_threads(),is_key_pressed(KEY_ACTION)&&(release_key(KEY_ACTION),state=STATE_GAME)):state==STATE_GAME?update_game_state()?(--ticks,0==
ticks&&(ticks=18,draw_panel_hunger(),--hunger,draw_panel_hunger(),0==hunger&&restart())):restart():state==STATE_COMPLETED&&(run_threads(),--player_direction,0==player_direction&&(state=STATE_TITLE));update_screen()};
